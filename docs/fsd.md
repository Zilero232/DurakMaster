...
| 5 | Entities | Core business concepts (session, settings, game) | Yes |
...
│   ├── _layout.tsx     # providers, fonts, language restoration
...
That is why `app/` contains **only** route files: `_layout.tsx`, `index.tsx`, `+not-found.tsx`. No `.styles.ts`, `.types.ts`, `components/` or helpers. Everything else goes into the layers next to `app/`, including the styles of the layout itself:
...
- `game/` — table, cards, the course of a round (`game-table`, `online-table`, `quick-phrases`, `view-discard`)
...
|---|---|---|
| Deck theme | `CardThemeProvider`, `useCardTheme`, `useSetCardTheme` | `app/_layout.tsx` — `initialValue` from the settings store |
| Press feedback | `FeedbackProvider`, `usePressFeedback` | `app/_layout.tsx` — sound + vibration from `shared/lib` |

```tsx
// app/_layout.tsx — the app knows about both the sound and the store; ui-kit knows about nothing
...
The rule: **a new dependency of `ui-kit` on the app is a new context, not a new import.** If a primitive needs something from the business layers, add a provider in `ui-kit/lib/` (or `ui-kit/theme/`, if it is presentation) and supply the value in `app/_layout.tsx`.

---

## 6. Integration with Expo Router

`app/` is the routes directory; route files are thin and delegate to `views/`:
...
Route files are the **only place with `export default`**: Expo Router requires exactly that, otherwise the screen is not found. Everything else in the project is exported by name.

`_layout.tsx` holds the providers (`GestureHandlerRootView`, `SafeAreaProvider`, `QueryClientProvider`, `Toaster`) and the loading of fonts and language — but not screen markup. The layout styles live in `views/root-layout` (see §1: a file placed next to it would become a route).
...
| A primitive in `ui-kit` imports `@/entities` or `@/shared` | Invert it: a provider in `ui-kit`, the value from `app/_layout.tsx` (§5.1) |
