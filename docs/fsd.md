...
│   └── <domain>/       # app | game | layout | lobby | social
│       └── <widget-name>/
├── features/           # Слой Features
│   └── <domain>/       # app | auth | game | lobby | social
│       └── <feature-name>/
...
**Доменные группы (DurakMaster):** слои `features/`, `entities/`, `widgets/` группируют слайсы по бизнес-домену:

- `auth/` — авторизация (sign-in, sign-up, user)
- `game/` — стол, карты, ход игры, таймеры
...
> Канон FSD рекомендует `export { Page as default } from '@/views/...'` и пустую `pages/` с `.gitkeep`. В DurakMaster слой назван `views/` — конфликта с Next.js Pages Router нет, `.gitkeep`-заглушка не нужна. Роут-обёртка пишется как обычный компонент (см. выше).
...
- Кодстайл DurakMaster поверх FSD (структура слайса, naming, размер компонента): [`docs/style.md`](./style.md)
