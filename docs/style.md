...
```ts
// ✓ ОК
import { LobbyHeader, LobbyList } from './components';
...
Так эффекты не раздувают тело главного компонента, каждый изолирован и тестируется отдельно. Альтернатива «куча `useEffect` в `GameTable.tsx`» — запрещена (превышает лимит 100 строк, секция 4).
...
export type GameTableProps = {
  tableId: string;
  seat: number;
  onActionRejected: (code: GameErrorCode) => void;
  onLeave: () => void;
};
```

**`GameTable.module.scss`:**
...
import s from './GameTable.module.scss';
import type { GameTableProps } from './GameTable.types';

export const GameTable = ({ token, serverUrl }: GameTableProps) => (
...
Тип входа хука называется `Use<Name>Input` (§5). Если он повторяет пропсы компонента — не дублируй, а выведи: `Pick<GameTableProps, 'roomId' | 'onLeave'>`.
...
| Что | Как | Пример |
|---|---|---|
| Слайсы | kebab-case | `game-table`, `lobby-panel` |
| Сегменты | kebab-case | `ui`, `model`, `lib`, `api`, `config` |
| Папка компонента | PascalCase | `GameTable/`, `LobbyFooter/` |
...
1. **Внешние value-импорты** — пакеты, `node:`-builtins, `@durak-master/*`.
...
import type { Room } from '@durak-master/schemas/rooms';
import type { GameTableProps } from './GameTable.types';
```
...
```ts
// ✗ ЗАПРЕЩЕНО
import { LobbyList } from '@/widgets/room/lobby-panel/ui/components/LobbyList';
import { Button } from '@/shared/ui/atoms/Button';

// ✓ ОК
import { LobbyPanel } from '@/widgets/room/lobby-panel';
...
```ts
// ui/GameTable/index.ts
export { GameTable } from './GameTable';
export type { GameTableProps } from './GameTable.types';
```
...
shared/api/
  ...
  tables/    ← joinTable / leaveTable / listTables
...
import type { CreateRoomRequest, Room } from '@durak-master/schemas';
...
Типы запроса/ответа — из `@durak-master/schemas` (тот же контракт, что валидирует NestJS). Функция возвращает `data`, ошибки летят исключением — их ловит React Query.
...
if (!trimmed) throw new Error('Table name required');

const accessToken = await getFreshAccessToken();
const { tableId, seat } = await joinTable({ name: trimmed }, accessToken);

writeSeatCache(tableId, { seat });
router.push(`/table?id=${encodeURIComponent(tableId)}`);
```
...
```ts
// ✓ ОК
import { createRoomInputSchema, type Room } from '@durak-master/schemas/rooms';
...
import { useForm } from 'react-hook-form';
import { createRoomInputSchema, type CreateRoomInput, type CreateRoomRawInput } from '@durak-master/schemas/rooms';
...
- Схема — в `@durak-master/schemas/<resource>`, не inline в форме.
...
return match(state)
  ...
  .with({ kind: 'active' }, ({ token, url }) => <GameTable token={token} serverUrl={url} />)
  .exhaustive();
```
...
return match({ roomId, isLoading, room, tokenData })
  ...
  .with({ tokenData: P.nonNullable, room: P.nonNullable }, ({ tokenData }) => (
    <GameTable serverUrl={tokenData.url} token={tokenData.token} />
  ))
  .exhaustive();
```
...
```tsx
// ✗ ПЛОХО — drilling
<LobbyList displayName={x} isAdmin={z} rooms={r} onDelete={d} />
...
- Данные приходят из `.map` (`<TableListItem table={table} />`).
- UI state оркестратора (`lobbyFiltersOpened` в `LobbyLayout`).
...
bun --filter @durak-master/client build          # сборка client
bun --filter @durak-master/server build          # сборка server
```
