...
import { DEFAULT_TYSYACHA_RULES, TYSYACHA_TRICK_POINTS_TOTAL } from '@durak-master/schemas';
...
describe('tysyacha deal settlement', () => {
  it('hands out all 120 card points and lets a bidder declare a marriage', () => {
    let state = newGame();

    let declaredMarriage = false;

    for (let steps = 0; steps < 2000 && state.stage !== 'scoring'; steps += 1) {
      const mover = USER_IDS.find(
      ...
      const action = decideBotAction(state, mover);

      if (action.type === 'declareMarriage') {
        declaredMarriage = true;
      }

      const result = reduce(state, mover, action);
    ...
    const taken = state.players.reduce(
      (total, player) => total + handPoints(state.wonCards[player.userId] ?? []),
      0
    );

    expect(taken).toBe(TYSYACHA_TRICK_POINTS_TOTAL);

    expect(declaredMarriage).toBe(true);
  });
});
